...
function renderCloud(ctx) {
  renderShape(ctx, SHADOW_COLOR, CLOUD_X + SHADOW_SHIFT, CLOUD_Y + SHADOW_SHIFT);
  renderShape(ctx, CLOUD_COLOR, CLOUD_X, CLOUD_Y);
}
...
function renderGreeting(ctx) {
  renderText(ctx, 'Ура вы победили!', CLOUD_X + GAP, CLOUD_Y + GAP + TEXT_GAP);
  renderText(ctx, 'Список результатов:', CLOUD_X + GAP, CLOUD_Y + GAP + TEXT_GAP * 2);
}
...
function renderColumn(ctx, columnX, columnHeight, columnColor, name, time) {
  renderText(ctx, time, columnX, COLUMN_BOTTOM - columnHeight - COLUMN_TEXT_UP_GUP);
  ctx.fillStyle = columnColor;
  ctx.fillRect(columnX, COLUMN_BOTTOM - columnHeight, COLUMN_WIDTH, columnHeight);
  renderText(ctx, name, columnX, COLUMN_BOTTOM + COLUMN_TEXT_BOTTOM_GUP);
}

function getColumnColor() {
  return 'hsl(254, ' + getRandomInRange(0, 100) + '%,' + ' 50%)';
}
...
function renderStatistics(ctx, names, times) {
  ...
  for (var i = 0; i < names.length; i++) {
    ...
    columnColor = (names[i] === 'Вы') ? COLUMN_PLAYER_COLOR : getColumnColor();
